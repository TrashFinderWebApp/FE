export default function LoginSVG() {
  return (
    <svg
      width="32"
      height="46"
      viewBox="0 0 32 46"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
    >
      <mask
        id="mask0_237_852"
        style={{ maskType: "alpha" }}
        maskUnits="userSpaceOnUse"
        x="0"
        y="0"
        width="32"
        height="44"
      >
        <path
          d="M9.96094 41.8867V42.6602H0.421875V41.8867H4.71094V39.918H1.61719V36.707H7.85156V35.0664H1.59375V34.3047H8.75391V37.457H2.53125V39.1562H9V39.918H5.61328V41.8867H9.96094ZM19.0078 34.6445V35.6875C19.0078 37 19.0078 38.4883 18.6211 40.75L17.6836 40.6562C18.1055 38.5234 18.1055 36.9648 18.1055 35.6875V35.4062H11.8242V34.6445H19.0078ZM20.2852 41.6523V42.4141H10.793V41.6523H20.2852ZM29.7656 33.543V41.1602H28.8516V33.543H29.7656ZM30.0938 42.9531V43.7266H23.0391V40.3867H23.9531V42.9531H30.0938ZM24.1406 34.2695C25.6992 34.2695 26.8711 35.3242 26.8711 36.8242C26.8711 38.3477 25.6992 39.3906 24.1406 39.3906C22.582 39.3906 21.4102 38.3477 21.4102 36.8242C21.4102 35.3242 22.582 34.2695 24.1406 34.2695ZM24.1406 35.0781C23.0859 35.0781 22.3008 35.8047 22.3125 36.8242C22.3008 37.8672 23.0859 38.582 24.1406 38.582C25.1719 38.582 25.9688 37.8672 25.9688 36.8242C25.9688 35.8047 25.1719 35.0781 24.1406 35.0781Z"
          fill="black"
        />
        <rect width="32" height="32" fill="url(#loginsvg)" />
      </mask>
      <g mask="url(#mask0_237_852)">
        <rect y="2" width="35" height="44" fill="#184E77" />
      </g>
      <defs>
        <pattern
          id="loginsvg"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlinkHref="#image0_237_852" transform="scale(0.0111111)" />
        </pattern>
        <image
          id="image0_237_852"
          width="90"
          height="90"
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
}
